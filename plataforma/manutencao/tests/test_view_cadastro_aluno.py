...
view_in_test = 'manutencao:cadastro_aluno'
template_in_test = 'cadastro_aluno.html'


class cadastro_aluno_NoAuthGet(TestCase):
    def setUp(self):
...
class cadastro_aluno_Get(TestCase, CreateTestUser):
    def setUp(self):
...
class cadastro_aluno_Post_Ok(TestCase, CreateTestUser):
    def setUp(self):
        data = self.create_user_admin()
        self.resp = self.client.post(r('core:login'), data)
    ...
    def test_user_profile(self):
        usuario = User.objects.filter(email__exact='orlando@saraiva.com')[0]
        self.assertTrue(usuario.is_student)
...
class cadastro_aluno_Post_Fail(TestCase, CreateTestUser):
    def setUp(self):
        data = self.create_user_admin()
        self.resp = self.client.post(r('core:login'), data)
...
class cadastro_empresa_access_denied_2(TestCase, CreateTestUser):
    def setUp(self):
...
class cadastro_empresa_access_denied_3(TestCase, CreateTestUser):
    def setUp(self):
...
class cadastro_empresa_access_denied_4(TestCase, CreateTestUser):
    def setUp(self):
